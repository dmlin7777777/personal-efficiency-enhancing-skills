...
**Step 5c-iii — Interview Defense Prep / 面试防御准备**:

For every 🔴 Major issue flagged in Step 5c-ii, generate 1-2 **mock interview follow-up questions** that the tailored wording might provoke. This transforms the audit from a passive check into active interview preparation.

```
For each 🔴 Major issue:

1. 🤖 AI feel / AI 化痕迹:
   If the issue is "overly polished phrasing" or "formulaic structure":
   → "面试官可能追问：'能具体讲讲这个项目的选型过程吗？为什么选 A 而不是 B？'"

2. 🕳️ Logical gaps / 逻辑跳跃:
   If the issue is "claiming outcomes without process":
   → "面试官可能追问：'你提到实现了 X% 的提升，能 walk me through 你是 how 量化这个 baseline 的吗？'"

3. 📏 Scope inflation / 角色范围夸大:
   If the issue is "升级了动词" (e.g., "参与" → "主导"):
   → "面试官可能追问：'请详细描述你主导此项目时遇到的最大资源冲突及解决方法。'"

4. 🎯 Buzzword defense / 术语自圆其说:
   If the issue is "methodology named without evidence":
   → "面试官可能追问：'你说你用了 A/B Testing 来优化策略，能说说当时的 control group 是怎么分的、sample size 多少吗？'"

5. 🌍 Cultural tone mismatch / 文化语调不匹配:
   If the issue is "overly aggressive for target region":
   → "面试官可能觉得过于自夸。准备 2-3 个 'we' 开头的协作案例来平衡 'I' 开头的个人成就。"
```

**Output** (appended to Step 5c-ii table or as a separate section):
```
## Interview Defense Prep / 面试追问预演

| # | Resume Wording / 简历措辞 | Potential Interview Question / 预期追问 | Prep Suggestion / 准备建议 |
|---|---|---|---|
| 1 | "主导了 X 项目的全链路..." | "请描述你主导此项目时遇到的最大资源冲突及解决方法" | 准备一个 STAR 格式的冲突解决故事 |
| 2 | "通过 A/B Testing 实现 30% 提升" | "Control group 如何划分？Sample size？Statistical significance？" | 准备实验设计的具体数据 |
```
