...
PII_PATTERNS = {
    ...
    "age": {
        ...
        "patterns": [
            ...
            r"\bDOB\s*[:：]\s*[\d/.\-]+\b",
            r"\d{4}\s*[-/年]\s*\d{1,2}\s*[-/月]\s*\d{1,2}\s*日?",  # full birth date
        ],
    },
    "gender": {
        "label": "Gender / 性别",
...
def check_date_consistency(text: str) -> list:
    ...
    date_formats = {
        "YYYY-MM": re.findall(r"\b\d{4}[-/]\d{1,2}\b", text),
        "YYYY.MM": re.findall(r"\b\d{4}\.\d{1,2}\b", text),
        "MMM YYYY": re.findall(
            r"\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)"
            r"[a-z]*\.?\s+\d{4}\b", text, re.IGNORECASE
        ),
        "MM/YYYY": re.findall(r"\b\d{1,2}/\d{4}\b", text),
        "YYYY年M月": re.findall(r"\b\d{4}\s*年\s*\d{1,2}\s*月\b", text),
        "YYYY": re.findall(r"\b(?:Present|present|Current|current|至今|现在)\b", text),
    }

    # Find which formats are actually used
    used_formats = {fmt: matches for fmt, matches in date_formats.items() if matches}
